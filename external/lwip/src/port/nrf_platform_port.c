 ...
 * @note That if ENABLE_DEBUG_LOG_SUPPORT is disabled, having NRF_DRIVER_ENABLE_LOGS has no effect.
 * @{
 */
#if (NRF_LWIP_DRIVER_ENABLE_LOGS == 1)

#define NRF_DRIVER_LOG  app_trace_log                           /**< Used for logging details. */
#define NRF_DRIVER_ERR  app_trace_log                           /**< Used for logging errors in the module. */
#define NRF_DRIVER_TRC  app_trace_log                           /**< Used for getting trace of execution in the module. */
#define NRF_DRIVER_DUMP app_trace_dump                          /**< Used for dumping octet information to get details of bond information etc. */

#else // NRF_LWIP_DRIVER_ENABLE_LOGS

#define NRF_DRIVER_DUMP(...)                                    /**< Diasbles dumping of octet streams. */
#define NRF_DRIVER_LOG(...)                                     /**< Diasbles detailed logs. */
#define NRF_DRIVER_ERR(...)                                     /**< Diasbles error logs. */
#define NRF_DRIVER_TRC(...)                                     /**< Diasbles traces. */

#endif // NRF_LWIP_DRIVER_ENABLE_LOGS
...
static void blenetif_input(struct netif  * p_netif,  uint8_t * p_payload, uint16_t payload_len)
{
    ...
    NRF_DRIVER_LOG("[IP-DRI]: >> blenetif_input\r\n");
    ...
    if (p_buffer != NULL)
    {
        ...
        {
            NRF_DRIVER_LOG("[IP-DRI]: IP Stack returned error.\r\n");
        }
...
static err_t blenetif_output(struct netif * p_netif, struct pbuf * p_buffer, const ip6_addr_t * p_addr)
{
    ...
    NRF_DRIVER_LOG("[IP-DRI]: >> blenetif_output\r\n");
    ...
    if (NULL != p_payload)
    {
        ...
        {
            NRF_DRIVER_ERR("[IP-DRI]: Failed to send IP packet, reason 0x%08X\r\n", retval);
            nrf_free(p_payload);
        }
        else
        {
    ...
    {
         NRF_DRIVER_ERR("[IP-DRI]: Failed to allocate memory for output packet\r\n");
    }
                                        ...
                                        ble_6lowpan_event_t * p_event)
{
    ...
    switch(p_event->event_id)
    {
        case BLE_6LO_EVT_INTERFACE_ADD:
        {
            ...
            {
                NRF_DRIVER_LOG("[IP-DRI]: Successfully allocated context table!\r\n");
            }
            else
            {
                NRF_DRIVER_ERR("[IP-DRI]: Failed to allocate context table!\r\n");
            }
        ...
        case BLE_6LO_EVT_INTERFACE_DELETE:
        {
            if (p_blenetif != NULL)
            {
                ...
                if(err_code == NRF_SUCCESS)
                {
                    NRF_DRIVER_ERR("[IP-DRI]: Successfully freed context table!\r\n");
                }
        ...
        case BLE_6LO_EVT_INTERFACE_DATA_RX:
        {
            ...
            {
                NRF_DRIVER_ERR("[IP-DRI]: Dropping packet, unknown interface.\r\n");
            }
            break;
        }
        default:
...
/**@brief  Message function to rediect LwIP debug traces to nRF tracing. */
